...
def graph_partitioning(G, plotting=True):
    ...
    H.remove_edges_from((n, nbr, d)
        for n, nbrs in G.adj.items() if n in node_supported_index
        for nbr, d in nbrs.items() if nbr in node_unsupported_index)
    H.remove_edges_from((n, nbr, d)
        for n, nbrs in G.adj.items() if n in node_unsupported_index
        for nbr, d in nbrs.items() if nbr in node_supported_index)
    H.graph.update(G.graph)

    G_minus_H = nx.DiGraph()
    # Collect all removed edges for reconstruction.
    G_minus_H.add_edges_from((n, nbr, d)
        for n, nbrs in G.adj.items() if n in node_supported_index
        for nbr, d in nbrs.items() if nbr in node_unsupported_index)
    G_minus_H.add_edges_from((n, nbr, d)
        for n, nbrs in G.adj.items() if n in node_unsupported_index
    ...
    for c in nx.connected_components(H.to_undirected()):
        ...
        if plotting:
            ...
            _node_color_list_c = ([v[1]['node_color']
                                   for v in H_c.nodes(data=True)])
...
G_ex.add_edges_from(([('In', 'A'), ('A', 'B'), ('B', 'C'), ('B', 'D'),
                      ('D', 'E'), ('C', 'F'), ('E', 'F'), ('F', 'Out')]))
...
# Unit test function for graph_partitioning
def test_graph_partitioning():
    ...
    _check = (nx.is_isomorphic(subgraphs_of_G_u[0], subgraphs_of_G_u_true[0])
              and
              nx.is_isomorphic(subgraphs_of_G_u[1], subgraphs_of_G_u_true[1]))
