...
logging.basicConfig(filename="proportional_allocation.log", level=logging.DEBUG)
...
def get_valuations_until_i(ordered_edges, vals_to_edges, edge, half):
    ...
    logger.info("Evaluate the path")
    for i in range(j + 1):
        len = vals_to_edges[ordered_edges[i]][0].cake_length()  # length of a single assignment of an edge
        for l in range(len):  # iterate over the piece
            logger.debug(f"Sum of path this far {s}")
            if s == half:
                return s, cake_alloc, l / len # if its exactly half we return the allocation and if we didnt take the whole edge assignment we return the part we stopped at
...
def get_proportional_allocation(v1, v2, graph):
    ...
    True
    """
    ordered_edges, oriented_edges = col.get_contiguous_oriented_labeling(graph) # get the orientation of the edges and the order
    ...
    i = 0
    edge_weight = 0
    for val in zip(v1, v2): # each value to each edge will be a tuple. First index value of 1st agent. Second to 2nd agent
    ...
    alloc2 = nx.Graph()
    alloc1 = tuple()
    logger.info("Get an increasing order path")
    for e in ordered_edges: # according to the algorithm, the knife goes from i- to i+ in increasing order of the labels.
        for path in nx.all_simple_paths(g, source=ordered_edges[0][0], target=e[1]): # compute an ordered directed path
            alloc1 = get_valuations_until_i(ordered_edges, vals_to_edges, (path[-2], path[-1]), sum_v1 / 2) # get the total value of it and then check if its half
            if alloc1[0] == sum_v1 / 2:
                u = path[-2]
                v = path[-1]
                part = alloc1[2]
                logger.info("Get the allocation for the 2nd agent")
                logger.debug(f"The allocation for agent 1 {alloc1}")
                for edge in g.edges:
                    if edge not in alloc1[1].edges:
                        ...
                        logger.debug(f"The allocation for agent 2 {alloc2}")
                if part != 1: # if part != 1 it means we need to add a fraction of an edge and not the entirety of it
