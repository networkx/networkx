...
Usually the `scipy.cluster.hierarchy.dendrogram` function is used to draw
dendrograms.  It receives in input a matrix which encodes the hierarchical
clustering to render as a dendrogram. For many cases it is enough to have this
matrix generated by the `scipy.cluster.hierarchy.linkage` function, but not so,
for example, in graph community detection.

The `dendrogram_from_girvan_newman` module provides functions that generate a
matrix of this type (named `agglomerative matrix`) starting from the community
reconstruction of a graph, performed with the 'girvan_newman' function  from
`networkx.algorithms.community.centrality`. The `agglomerative matrix` is
suitable to be taken in input by the `scipy.cluster.hierarchy.dendrogram`
function.
"""
...
from networkx.algorithms.community import dendrogram_from_girvan_newman as dgn
