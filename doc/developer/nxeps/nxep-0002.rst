...
- **Improved Exceptions** - Currently, users see the following exception when
  ...
  A more specific exception message could be something along the lines of::
     ...
     Traceback (most recent call last)
        ...
     Error: NodeView does not support slicing. Try list(G.nodes)[0:10].
...
discussion above. Users will now see `Error` when they try to slice a
`*View` object.::

   >>> G.edges()[0:10]
   Traceback (most recent call last)
      ...
   Error: EdgeView does not support slicing, try list(G.edges)[0:10:None]
