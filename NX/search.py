...
def dfs_preorder(G,v=None):
    ...
    """
    V=SC.Preorder(G, queue=NX.queues.DFS)
    V.search(v)
    return V.forest
...
def dfs_postorder(G,v=None):
    ...
    Optional v=v limits search to component of graph containing v.
    """
    V=SC.Postorder(G,queue=NX.queues.DFS)
    V.search(v)
    return V.forest
...
def dfs_predecessor(G,v=None):
    ...
    Optional v=v limits search to component of graph containing v.
    """
    V=SC.Predecessor(G,queue=NX.queues.DFS)
    V.search(v)
    return V.data
...
def bfs_length(G,source,target=None):
    ...
    """
    V=SC.Length(G,queue=NX.queues.BFS)
    V.search(source)
    if target!=None:
        try:
...
def bfs_path(G,source,target=None):
    ...
    """
    V=SC.Predecessor(G,queue=NX.queues.BFS)
    V.search(source)
    if target!=None:
        path=V.path(target)
...
def dfs_forest(G,v=None):
    ...
    """
    V=SC.Forest(G,queue=NX.queues.DFS)
    V.search(v)
    return V.forest
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/search.txt',package='NX')
    return suite
