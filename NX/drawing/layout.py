...
import sys
#from NX.base import Network
from NX.utils import uniform_sequence
try:
    import Numeric as N
except ImportError:
...
#        sys.stderr.write("drawing")
        while difference>eps:
            ...
            if norm==0:
                raise NX.NetworkXError,"Eigenvector with zero eigenvalue given as input."
            nn=1.0/norm
            for k in vv:
                vv[k] *= nn
            ##
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/layout.txt',package='NX')
    return suite
