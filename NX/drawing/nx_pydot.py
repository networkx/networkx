...
def pydot_from_NX(N):
	...
	# node_orig = 1
        ...
        # FIXME - add properties from NX to pydot graph?
        # a nice feature would be to add node positions to pydot graph.
        return graph

def NX_from_pydot(D, result=False):
	"""Creates an NX graph from an pydot graph D"""
        ...
        # FIXME - add properties from pydot to NX?
	return N
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/nx_pydot.txt',package='NX')
    return suite
