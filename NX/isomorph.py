...
def graph_could_be_isomorphic(G1,G2):
    ...
    t1=NX.cluster.triangles(G1,with_labels=True)
    c1=NX.cliques.number_of_cliques(G1,with_labels=True)
    props1=[ [d1[v], t1[v], c1[v]] for v in d1 ]
    props1.sort()
    
    d2=G2.degree(with_labels=True)
    t2=NX.cluster.triangles(G2,with_labels=True)
    c2=NX.cliques.number_of_cliques(G2,with_labels=True)
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/isomorph.txt',package='NX')
    return suite
