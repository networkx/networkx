...
#from NX.base import *
import NX
from NX.utils import is_string_like
...
def union(G,H,create_using=None,rename=False,**kwds):
    ...
    for n in Gname.values():
        if n in Hname.values():
            raise NX.NetworkXError,"node sets of G and H are not disjoint. Use appropriate rename=('Gprefix','Hprefix')"
...
def disjoint_union(G,H):
    ...
    R2=convert_node_labels_to_integers(H,first_label=NX.number_of_nodes(R1)+1)
    R=union(R1,R2)
...
#           print v_int, "-->", v
        ...
        raise NX.NetworkXError,"unknown value of node ordering variable: ordering"
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/operators.txt',package='NX')
    return suite
