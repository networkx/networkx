...
import random
import NX
from NX.generators.classic import empty_graph, path_graph, complete_graph
...
def random_shell_graph(constructor,seed=None):
    ...
    for (n,m,d) in constructor:
        ...
        g=NX.operators.convert_node_labels_to_integers(
...
def random_powerlaw_tree_sequence(n, gamma=3, seed=None, tries=100):
    ...
    raise NX.NetworkXError, "Exceeded max (%d) attempts for a valid tree sequence."%tries
    return False
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/generators_random_graphs.txt',package='NX')
    return suite
