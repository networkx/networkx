...
def balanced_tree(r,h):
    ...
    """
    if r<2:
        raise NX.NetworkXError, "Invalid graph description, r should be >=2"
    if h<1:
        raise NX.NetworkXError, "Invalid graph description, h should be >=1"
...
def grid_graph(dim,periodic=False):
    ...
    """    
    from NX.utils import is_list_of_ints
...
def _test_suite():
    import doctest
    suite = doctest.DocFileSuite('tests/generators_classic.txt',package='NX')
    return suite
