...
default_flow_func = preflow_push_value
...
def maximum_flow(G, s, t, capacity='capacity', flow_func=None):
    ...
    --------
    minimum_cut
    :func:`networkx.algorithms.flow.ford_fulkerson_value`
    :func:`networkx.algorithms.flow.preflow_push_value`
    :func:`networkx.algorithms.flow.augmenting_shortest_path_value`
    ...
    >>> flow_value
    3.0
    >>> # If you want to use alternative algorithms, you have to
    >>> # import them explicitly from the flow package.
    >>> from networkx.algorithms.flow import shortest_augmenting_path_value
    >>> assert(flow_value == 
    ...         nx.maximum_flow(G, 'x', 'y', 
    ...                         flow_func=shortest_augmenting_path_value))
...
def minimum_cut(G, s, t, capacity='capacity', flow_func=None):
    ...
    :func:`networkx.algorithms.flow.preflow_push_value`
    :func:`networkx.algorithms.flow.ford_fulkerson_value`
    :func:`networkx.algorithms.flow.shortest_aumenting_path_value`
    ...
    >>> G.add_edge('c','y', capacity = 2.0)
    >>> G.add_edge('e','y', capacity = 3.0)
    >>> min_cut = nx.minimum_cut(G, 'x', 'y')
    >>> min_cut
    3.0
    >>> # If you want to use alternative algorithms for computing
    >>> # the maximum flow value, you have to import them
    >>> # explicitly from the flow package.
    >>> from networkx.algorithms.flow import shortest_augmenting_path_value
    >>> assert(min_cut == 
    ...         nx.minimum_cut(G, 'x', 'y', 
    ...                         flow_func=shortest_augmenting_path_value))
