...
class TestNetworkSimplex:
    ...
    def test_neg_infinite_demand_raise(self):
        G = nx.DiGraph()
        inf = float("inf")
        G.add_node("a", demand=-inf)
        G.add_node("b", demand=-5)
    ...
    def test_infinite_weight_raise(self):
        G = nx.DiGraph()
        inf = float("inf")
        ...
        G.add_node("c", demand=50000000)
        G.add_node("d", demand=-49999995)
        G.add_edge("a", "b", weight=inf, capacity=4)
        G.add_edge("a", "c", weight=inf, capacity=10)
        G.add_edge("b", "d", weight=1, capacity=9)
        G.add_edge("c", "d", weight=2, capacity=5)
        pytest.raises(nx.NetworkXError, nx.network_simplex, G)

    def test_nonzero_net_demand_raise(self):
    ...
    def test_google_or_tools_example(self):
        """
