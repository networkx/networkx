def find_candidates(G1, G2, G1_labels, G2_labels, u, state):
    ...
    if len(covered_neighbors) == 0:
        return {node for node in G2.nodes() if
                node not in state.reverse_mapping and G2.degree[node] == G1.degree[u] and G2_labels[node] == G1_labels[
                    u] and len([nbr2 for nbr2 in G2[node] if nbr2 in state.reverse_mapping]) == 0}
