...
class GraphMatcher(object):
    ...
    def candidate_pairs_iter(self):
        ...
        T1_inout = [
            node for node in G1_nodes if (
                node in self.inout_1) and (
                node not in self.core_1)]
        T2_inout = [
            node for node in G2_nodes if (
                node in self.inout_2) and (
                node not in self.core_2)]
        ...
        if T1_inout and T2_inout:
            ...
            # if not (T1_inout or T2_inout):       # as suggested by  [2],
            # incorrect
...
#    __doc__ += "Notes\n%s-----" % (indent,) + sources.replace('\n','\n'+indent)
    ...
    def candidate_pairs_iter(self):
        ...
        T1_out = [
            node for node in G1_nodes if (
                node in self.out_1) and (
                node not in self.core_1)]
        T2_out = [
            node for node in G2_nodes if (
                node in self.out_2) and (
                node not in self.core_2)]
        ...
        # elif not (T1_out or T2_out):   # as suggested by [2], incorrect
            ...
            T1_in = [
                node for node in G1_nodes if (
                    node in self.in_1) and (
                    node not in self.core_1)]
            T2_in = [
                node for node in G2_nodes if (
                    node in self.in_2) and (
                    node not in self.core_2)]
    ...
    def syntactic_feasibility(self, G1_node, G2_node):
        ...
        for predecessor in self.G1.pred[G1_node]:
            if (predecessor in self.out_1) and (
                    predecessor not in self.core_1):
                num1 += 1
        num2 = 0
        for predecessor in self.G2.pred[G2_node]:
            if (predecessor in self.out_2) and (
                    predecessor not in self.core_2):
                num2 += 1
        if self.test == 'graph':
            if not (num1 == num2):
                return False
        ...
        num1 = 0
        for predecessor in self.G1.pred[G1_node]:
            if (predecessor not in self.in_1) and (
                    predecessor not in self.out_1):
                num1 += 1
        num2 = 0
        for predecessor in self.G2.pred[G2_node]:
            if (predecessor not in self.in_2) and (
                    predecessor not in self.out_2):
                num2 += 1
        if self.test == 'graph':
            if not (num1 == num2):
                return False
...
class DiGMState(object):
    ...
    def __init__(self, GM, G1_node=None, G2_node=None):
        ...
        if G1_node is not None and G2_node is not None:
            ...
            new_nodes = set([])
            for node in GM.core_1:
                new_nodes.update([predecessor for predecessor in GM.G1.predecessors(
                    node) if predecessor not in GM.core_1])
            for node in new_nodes:
            ...
            new_nodes = set([])
            for node in GM.core_2:
                new_nodes.update([predecessor for predecessor in GM.G2.predecessors(
                    node) if predecessor not in GM.core_2])
            for node in new_nodes:
            ...
            for node in GM.core_1:
                new_nodes.update(
                    [successor for successor in GM.G1.successors(node) if successor not in GM.core_1])
            for node in new_nodes:
            ...
            for node in GM.core_2:
                new_nodes.update(
                    [successor for successor in GM.G2.successors(node) if successor not in GM.core_2])
            for node in new_nodes:
    ...
    def restore(self):
        ...
        # Thus, we delete all entries which have this depth level.
        for vector in (
                self.GM.in_1, self.GM.in_2, self.GM.out_1, self.GM.out_2):
            for node in list(vector.keys()):
                if vector[node] == self.depth:
                    del vector[node]
