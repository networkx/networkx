...
def _period(G):
    ...
    # Only compute period for the first SCC
    # The rest are processed in the recursive call
    scc = next(nx.strongly_connected_components(G))

    s = arbitrary_element(scc)
    levels = {s: 0}
    this_level = [s]
    g = 0
    lev = 1
    while this_level:
        ...
        for u in this_level:
            for v in G[u]:
                ...
                # Tree Edge
                # `v in scc` ensures we do not go outside the current SCC
                elif v in scc:
                    next_level.append(v)
                    levels[v] = lev
    ...
    if len(levels) == len(G):  # All nodes in tree
        ...
        return gcd(g, _period(G.subgraph(set(G) - scc)))
