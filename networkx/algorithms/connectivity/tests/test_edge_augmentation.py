...
# def debug(G, C, T, D, A, rooted, aug_edges, candidate_mapping):
#     # locals().update()
#     # ut.qtensure()
...
#         D, name='label', values=ut.map_vals(
#             lambda w: '{:.2f}'.format(w),
#             nx.get_edge_attributes(D, 'weight')))
#     # nx.set_edge_attributes(
...
#     import utool as ut
#     nx.set_edge_attributes(
#         D_, name='label', values=ut.map_vals(
#             lambda w: '{:.2f}'.format(w),
#             nx.get_edge_attributes(D_, 'weight')))
...
#     C2 = C.copy()
#     C2.add_edges_from([m.meta_uv for m in candidate_mapping], implicit=True)
