...
xdoctest -m networkx.algorithms.string.balanced_sequence_cython list
python -m xdoctest networkx.algorithms.string.balanced_sequence_cython __doc__:0 --bench
...
>>> seq_len = 800
>>> seq1, open_to_close1 = bseq.random_balanced_sequence(seq_len, item_type='paren')
>>> seq2, open_to_close2 = bseq.random_balanced_sequence(seq_len, item_type='paren')
>>> open_to_close = {**open_to_close1, **open_to_close2}
>>> n = 1
>>> ti = timerit.Timerit(n, bestof=max(2, n), verbose=2)
>>> for timer in ti.reset('impl=iter-cython'):
>>>     with timer:
>>>         bseq.longest_common_balanced_sequence(seq1, seq2, open_to_close, impl='iter-cython')
>>> for timer in ti.reset('impl=iter'):
>>>     with timer:
>>>         bseq.longest_common_balanced_sequence(seq1, seq2, open_to_close, impl='iter')
