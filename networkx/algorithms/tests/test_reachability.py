"""Tests for the PLL reachability graph (``nx.pll_reachability_graph``)."""
...
def _first_mismatch_vs_has_path(G):
    ...
    for s in G:
        for t in G:
            if nx.is_reachable(H, s, t) != nx.has_path(G, s, t):
                return (s, t, nx.is_reachable(H, s, t), nx.has_path(G, s, t))
    return None
...
class TestKnownAnswers:
    ...
    def test_simple_path(self):
        ...
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 3) is True
        assert nx.is_reachable(H, 3, 0) is False
        assert nx.is_reachable(H, 1, 2) is True
        assert nx.is_reachable(H, 0, 0) is True  # a node reaches itself

    def test_branching_dag(self):
        G = nx.DiGraph([(0, 1), (0, 2), (1, 3), (2, 3)])
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 3) is True
        assert nx.is_reachable(H, 1, 2) is False
        assert nx.is_reachable(H, 3, 0) is False
    ...
    def test_cycle_is_mutually_reachable(self):
        ...
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 2) is True
        assert nx.is_reachable(H, 2, 0) is True  # reachable around the cycle
        assert nx.is_reachable(H, 1, 3) is True
        assert nx.is_reachable(H, 3, 0) is False
    ...
    def test_string_node_labels(self):
        ...
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, "a", "c") is True
        assert nx.is_reachable(H, "c", "a") is False
...
class TestEdgeCases:
    ...
    def test_single_node(self):
        ...
        G.add_node(0)
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 0) is True

    def test_self_loop(self):
        G = nx.DiGraph([(0, 0), (0, 1)])
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 1) is True
        assert nx.is_reachable(H, 1, 0) is False
        assert nx.is_reachable(H, 0, 0) is True
    ...
    def test_isolated_nodes(self):
        ...
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 0) is True
        assert nx.is_reachable(H, 0, 1) is False

    def test_disconnected_components(self):
        G = nx.DiGraph([(0, 1), (1, 2), (10, 11)])
        H = nx.pll_reachability_graph(G)
        assert nx.is_reachable(H, 0, 2) is True
        assert nx.is_reachable(H, 0, 10) is False
        assert nx.is_reachable(H, 10, 11) is True
    ...
    def test_single_scc(self):
        ...
        for s in range(6):
            for t in range(6):
                assert nx.is_reachable(H, s, t) is True
...
class TestValidation:
    def test_missing_source_raises(self):
        H = nx.pll_reachability_graph(nx.DiGraph([(0, 1)]))
        with pytest.raises(nx.NodeNotFound):
            nx.is_reachable(H, 99, 1)

    def test_missing_target_raises(self):
        H = nx.pll_reachability_graph(nx.DiGraph([(0, 1)]))
        with pytest.raises(nx.NodeNotFound):
            nx.is_reachable(H, 0, 99)
