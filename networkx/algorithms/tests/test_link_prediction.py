...
class TestResourceAllocationIndex:
    ...
    def test_notimplemented(self):
        assert pytest.raises(
            nx.NotImplemented, self.func, nx.DiGraph([(0, 1), (1, 2)]), [(0, 2)]
        )
        assert pytest.raises(
            nx.NotImplemented,
            self.func,
...
class TestCommonNeighborCentrality:
    ...
    def test_notimplemented(self, graph_type):
        assert pytest.raises(
            nx.NotImplemented, self.func, graph_type([(0, 1), (1, 2)]), [(0, 2)]
        )
    ...
    def test_equal_nodes(self):
        G = nx.complete_graph(4)
        assert pytest.raises(nx.AlgorithmError, self.test, G, [(0, 0)], [])
...
class TestCNSoundarajanHopcroft:
    ...
    def test_notimplemented(self):
        ...
        assert pytest.raises(nx.NotImplemented, self.func, G, [(0, 2)])
    ...
    def test_no_community_information(self):
        G = nx.complete_graph(5)
        assert pytest.raises(nx.AlgorithmError, list, self.func(G, [(0, 1)]))
    ...
    def test_insufficient_community_information(self):
        ...
        G.nodes[1]["community"] = 0
        G.nodes[3]["community"] = 0
        assert pytest.raises(nx.AlgorithmError, list, self.func(G, [(0, 3)]))
...
class TestWithinInterCluster:
    ...
    def test_invalid_delta(self):
        G = nx.complete_graph(3)
        G.add_nodes_from([0, 1, 2], community=0)
        assert pytest.raises(nx.AlgorithmError, self.func, G, [(0, 1)], 0)
        assert pytest.raises(nx.AlgorithmError, self.func, G, [(0, 1)], -0.5)
