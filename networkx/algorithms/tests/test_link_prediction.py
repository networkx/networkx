...
class TestDirectIndirectCommonNeighbors:
    @classmethod
    def setup_class(cls):
        cls.func = staticmethod(nx.direct_indirect_common_neighbors)
        cls.test = partial(_test_func, predict_func=cls.func)

    def test_K5(self):
        G = nx.complete_graph(5)
        self.test(G, [(0, 1)], [(0, 1, 4.0)])

    def test_P3(self):
        G = nx.path_graph(3)
        self.test(G, [(0, 2)], [(0, 2, 2.0)])

    def test_S4(self):
        G = nx.star_graph(4)
        self.test(G, [(1, 2)], [(1, 2, 2.0)])
