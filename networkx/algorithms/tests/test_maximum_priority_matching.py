...
class Test_maximum_priority_matching:
    def test_find_prioirity_score(self):
        ...
        assert find_priority_score(G, matching) == "000000000"
    ...
    def test_find_augmenting_paths(self):
        ...
        assert find_augmenting_paths(G, 1) == True
        assert prepare_for_algo(G, 1) == ([("8", "7")], ["8"])
        assert find_augmenting_paths(G, 1) == True
        assert prepare_for_algo(G, 1) == ([], [])
        assert find_augmenting_paths(G, 1) == False
        assert prepare_for_algo(G, 2) == ([], [])
        assert find_augmenting_paths(G, 2) == False
        assert prepare_for_algo(G, 3) == ([], [])
        assert find_augmenting_paths(G, 3) == False
        assert prepare_for_algo(G, 4) == (
            [("6", "5"), ("6", "7")],
            ["6"],
        )
        assert find_augmenting_paths(G, 4) == True
        assert prepare_for_algo(G, 4) == ([], [])
        assert find_augmenting_paths(G, 4) == False
        assert prepare_for_algo(G, 5) == (
            [("4", "3"), ("4", "5")],
            ["4"],
        )
        assert find_augmenting_paths(G, 5) == True
        assert prepare_for_algo(G, 5) == ([], [])
        assert find_augmenting_paths(G, 5) == False
        assert prepare_for_algo(G, 6) == ([], [])
        assert find_augmenting_paths(G, 6) == False
        assert prepare_for_algo(G, 7) == ([("9", "7")], ["9"])
        assert find_augmenting_paths(G, 7) == False
        assert prepare_for_algo(G, 8) == ([], [])
    ...
    def test_find_blossom(self):
        ...
        assert find_blossom(G, blossoms, "10", "12") == (
            {
                "nodes": ["10", "11", "12"],
                "root": "8",
                "isPositive": True,
        ...
        assert shrink_graph(G, blossoms["B1"], "B1") == [
            (False, "B1"),
            (False, "B1"),
            (False, "B1"),
        ]
    ...
    def test_find_path_to_base(self):
        ...
        u = "7"
        base = "3"
        assert paths_to_base(list, u, base) == (
            ["7", "5", "4", "3"],
            ["7", "6", "3"],
        )
        ...
        assert paths_to_base(["1", "2", "3"], "2", "1") == (
            ["2", "3", "1"],
            ["2", "1"],
        )
        assert paths_to_base(["1", "2", "3"], "1", "1") == (
            ["1"],
            ["1"],
        )
        assert paths_to_base(["1", "2", "3"], "3", "1") == (
            ["3", "1"],
            ["3", "2", "1"],
        )
