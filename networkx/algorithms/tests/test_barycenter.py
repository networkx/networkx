...
class TestBarycenter(TestCase):
	...
	def get_barycenter_and_assert(self, g, **kwargs):
		"""Take the graph, assert :func:`barycenter`'s return type, and return subgraph."""
	...
	def test_trees(self):
		...
		for i in range(50):
			with self.subTest(trial=i):
				b = self.get_barycenter_and_assert(
	...
	def test_this_one_specific_tree(self):
		...
		g = networkx.Graph({
			...
			'z': ['y', 0, 1, 2, 3, 4],
			0: ['z'], 1: ['z'], 2: ['z'], 3: ['z'], 4: ['z']})
		b = self.get_barycenter_and_assert(g, attr='barycentricity')
		...
		b = self.get_barycenter_and_assert(g, weight='weight', attr='barycentricity2')
		self.assertEqual(list(b), ['z'])
		self.assertFalse(b.edges)
		for node, barycentricity in expected_barycentricity.items():
