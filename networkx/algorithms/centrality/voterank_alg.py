...
def vote_rank(G, number_of_nodes=None):
    """Select a list of influential nodes in a graph using VoteRank algorithm
    ...
    -------
    voterank : dict
        Unordered dictionary with nodes as a key and vote as a value.
        Only nodes with positive number of votes are returned.
    ...
    for _ in range(number_of_nodes):
        ...
        for n, nbr in G.edges():
            ...
            if not G.is_directed():
                vote_rank[nbr][0] += vote_rank[n][1]
        for n, v in influential_nodes.items():
            vote_rank[n][0] = 0
        # step 3 - select top node
        n = max(G.nodes, key=lambda x: vote_rank[x][0])
        if vote_rank[n][0] == 0:
            return influential_nodes
        influential_nodes[n] = vote_rank[n][0]
        # weaken the selected node
        vote_rank[n] = [0, 0]
        # step 4 - update voterank properties
        for _, nbr in G.edges(n):
