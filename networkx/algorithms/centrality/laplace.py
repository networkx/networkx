...
def laplacian_centrality(G,normalized=True,nbunch=None,directed_laplacian_matrix_args=None):
    ...
    >>> laplacian_centrality(nxg)
    {0: 0.6999999999999997, 
    1: 0.8999999999999999, 
    2: 0.2799999999999998, 
    3: 0.21999999999999964, 
    4: 0.26, 
    5: 0.03999999999999948}
    ...
    if G.is_directed():
        lap_matrix = sparse.csr_matrix(nx.directed_laplacian_matrix(G,**directed_laplacian_matrix_args))
    else: 
        lap_matrix = nx.laplacian_matrix(G)
    
    if  normalized:
        sum_of_full = np.power(linalg.eigh(lap_matrix.toarray(),eigvals_only = True),2).sum()
    else:
        sum_of_full=1
    ...
    for i,v in enumerate(vs):   
        ...
        sum_of_eigen_values_2 = np.power(linalg.eigh(A_2.toarray(),eigvals_only = True),2).sum()
