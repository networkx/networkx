...
def domirank(G, analytical = False, sigma = 0.95, dt = 0.1, epsilon = 1e-5, max_iter = 1000, check_step = 10):
    ...
    if analytical == False:
        ...
        sigma = np.abs(sigma/_find_smallest_eigenvalue(GAdj, maxDepth = int(max_iter/5), dt = dt, epsilon = epsilon, max_iter = max_iter, check_step = check_step))
        ...
        if (N>5000):
            warnings.warn("The system is large!!! Consider using <analytical = False> function argument for reduced computational time cost.")
        # Here we create another warning for sigma being supercharged
        if (sigma > 1):
            warnings.warn("You are supercharging the competition in the system by having sigma > 1, which is only permitted for the analytical solution!")
        if sigma < 0:
            raise nx.NetworkXPointlessConcept(
                "the competition parameter sigma must be positive - sigma > 0."
            )
        sigma = np.abs(sigma/_find_smallest_eigenvalue(GAdj, maxDepth = int(max_iter/5), dt = dt, epsilon = epsilon, max_iter = max_iter, check_step = check_step))
        Psi = sp.sparse.linalg.spsolve(sigma*GAdj + sp.sparse.identity(GAdj.shape[0]), sigma*GAdj.sum(axis=-1))
        Psi =  dict(zip(G, (Psi).tolist()))
        return Psi, sigma, True
...
def _find_smallest_eigenvalue(G, minVal = 0, maxVal = 1, maxDepth = 100, dt = 0.1, epsilon = 1e-5, max_iter = 100, check_step = 10):
    '''
    ...
    for i in range(maxDepth):
        if maxVal - minVal < epsilon:
            break
        if _domirank(G, sigma = x, dt = dt, epsilon = epsilon, max_iter = max_iter, check_step = check_step):
            minVal = x
            x = (minVal + maxVal)/2
            minValStored = minVal
        else:
