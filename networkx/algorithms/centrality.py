...
def information_centrality(G,v=None,normalized=True,weighted_edges=None):
    """Compute information centrality for nodes.
    ...
    v : node, optional
      If not None, then return the information centrality for v only.
          
    normalized : bool, optional
      If True, return normalized information centrality values.  Values are
      normalized by the sum of the information centrality values for all nodes
      in G.
       
    weighted : bool | function, optional
      If True, edge weights are used to determine information centrality. If 
      weighted is None, then G.weighted determines the value of this parameter.  
      To use weights when edge data are not explicitly weights, a function
      may be specified instead.  The function should accept the edge data as
      its sole required argument and should return the weight to be used by 
      the algorithm.

    Returns
    -------
    nodes : dictionary
       Dictionary of nodes with information centrality as values.
    ...
    Notes
    ------
    Information centrality requires an undirected, connected graph. In
    principle, it should be possible to extend this technique to directed 
    graphs, but the details need to be worked out.

    Implementation is exact and dense, following from:
    ...
    if type(G) == nx.MultiGraph or type(G) == nx.MultiDiGraph():
        m = "information_centrality() not defined for graphs with multiedges."
        raise(Exception(m))

    if G.directed:
        m = "Undirected graphs only."
        raise(Exception(m))

    if not nx.is_connected(G):
        m = "Connected graphs only."
        raise(Exception(m))

    if v is not None and v not in G:
        m = "Node %s is not in G." % (v,)
        raise(Exception(m))
    ...
    if weighted:
        if not hasattr(weighted, __call__):
            # weighted parameter is not callable
            weight = lambda x : x
    else:
        weight = lambda x : 1.0
    ...
    # specifies the row/col order
    nodes = G.nodes()
    n = len(nodes)

    if not weighted:
        # B = D(r) - A + J
        # D(r) is diagonal matrix of degrees
        # A is adjacency matrix
        # J is matrix of ones
        degrees = np.asarray(nx.degree(G, nodes), dtype=float)
        D = np.diag(degrees)
        A = np.asarray(nx.adj_matrix(G, nodes))
        ...
        B = np.ones((n,n))
        for i in range(n):
            # count backwards so we can handle i==j at the end
            total = 0.0
            for j in range(n-1,i-1,-1):
                if i == j:
                    B[i,j] = 1 + total
                else:
                    edge = G.get_edge_data(i,j, default=None)
                    if edge is not None:
                        w = weight(edge)
                        total += w
                        B[i,j] = 1 - w
                    # if edge is None, B[i,j] should be 1
                    # and this is exactly how we initialized B
                  
    C = linalg.inv(B)
    ic = 1.0 / (np.diag(C) + (np.trace(C) - 2 * C[0].sum()) / n)
