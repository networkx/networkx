...
def _make_overlap_graph():
    ...
    Overlap(A,B): {C}=1, Overlap(B,C): {A, D}=2
    """
    G = nx.Graph()
...
# ========================================================================
# Tests: Neighborhood overlap (moved from test_backbone.py)
# ========================================================================
...
class TestNeighborhoodOverlap:
    ...
    def test_path_graph_no_overlap(self):
        """Path graph: adjacent nodes have 0 common neighbors."""
        G = nx.path_graph(3)
        H = neighborhood_overlap(G)
...
# ========================================================================
# Tests: Jaccard backbone (moved from test_backbone.py)
# ========================================================================
...
# ========================================================================
# Tests: Dice backbone (moved from test_backbone.py)
# ========================================================================
...
# ========================================================================
# Tests: Cosine backbone (moved from test_backbone.py)
# ========================================================================
...
# ========================================================================
# Tests: Cross-method relationships (moved from test_backbone.py)
# ========================================================================

class TestNeighborhoodOverlapRelationships:
    """Cross-method properties that should hold across the four original measures."""
...
# ========================================================================
# Tests: Hub Promoted Index
# ========================================================================

class TestHubPromotedIndex:
    """Tests for Hub Promoted Index."""

    def test_hpi_added(self):
        G = _make_overlap_graph()
    ...
    def test_hpi_values(self):
        G = _make_overlap_graph()
        H = hub_promoted_index(G)
        # A-B: overlap=1, min(deg(A)=2, deg(B)=3) = 2 → 1/2 = 0.5
        assert H["A"]["B"]["hpi"] == pytest.approx(0.5)
        # B-C: overlap=2, min(deg(B)=3, deg(C)=3) = 3 → 2/3
        assert H["B"]["C"]["hpi"] == pytest.approx(2.0 / 3.0)
        # C-D: overlap=1, min(deg(C)=3, deg(D)=2) = 2 → 1/2
...
# ========================================================================
# Tests: Hub Depressed Index
# ========================================================================

class TestHubDepressedIndex:
    """Tests for Hub Depressed Index."""

    def test_hdi_added(self):
        G = _make_overlap_graph()
    ...
    def test_hdi_values(self):
        G = _make_overlap_graph()
        H = hub_depressed_index(G)
        # A-B: overlap=1, max(deg(A)=2, deg(B)=3) = 3 → 1/3
        assert H["A"]["B"]["hdi"] == pytest.approx(1.0 / 3.0)
        # B-C: overlap=2, max(deg(B)=3, deg(C)=3) = 3 → 2/3
        assert H["B"]["C"]["hdi"] == pytest.approx(2.0 / 3.0)
        # C-D: overlap=1, max(deg(C)=3, deg(D)=2) = 3 → 1/3
...
# ========================================================================
# Tests: LHN Local Index
# ========================================================================

class TestLHNLocalIndex:
    """Tests for Leicht-Holme-Newman local similarity index."""

    def test_lhn_added(self):
        G = _make_overlap_graph()
        H = lhn_local_index(G)
        for u, v, d in H.edges(data=True):
    ...
    def test_lhn_values(self):
        G = _make_overlap_graph()
        H = lhn_local_index(G)
        # A-B: overlap=1, deg(A)=2 * deg(B)=3 = 6 → 1/6
        assert H["A"]["B"]["lhn_local"] == pytest.approx(1.0 / 6.0)
        # B-C: overlap=2, deg(B)=3 * deg(C)=3 = 9 → 2/9
...
# ========================================================================
# Tests: Preferential Attachment Score
# ========================================================================

class TestPreferentialAttachmentScore:
    """Tests for preferential attachment score."""

    def test_pa_added(self):
        G = _make_overlap_graph()
    ...
    def test_pa_values(self):
        G = _make_overlap_graph()
        H = preferential_attachment_score(G)
        # A-B: deg(A)=2 * deg(B)=3 = 6
        assert H["A"]["B"]["pa"] == 6
        # B-C: deg(B)=3 * deg(C)=3 = 9
        assert H["B"]["C"]["pa"] == 9
        # B-D: deg(B)=3 * deg(D)=2 = 6
        assert H["B"]["D"]["pa"] == 6
...
class TestAdamicAdarIndex:
    ...
    def test_aa_values(self):
        ...
        # B-C: common = {A, D}, deg(A)=2, deg(D)=2 → 1/log(2) + 1/log(2) = 2/log(2)
    ...
    def test_complete_graph(self):
        ...
        H = adamic_adar_index(G)
        for u, v, d in H.edges(data=True):
            # 2 common neighbors, each with degree 3 → 2/log(3)
...
# ========================================================================
# Tests: Resource Allocation Index
# ========================================================================

class TestResourceAllocationIndex:
    """Tests for resource allocation index."""

    def test_ra_added(self):
        G = _make_overlap_graph()
    ...
    def test_complete_graph(self):
        ...
        H = resource_allocation_index(G)
        for u, v, d in H.edges(data=True):
            # 2 common neighbors each with degree 3 → 2/3
...
# ========================================================================
# Tests: Graph Distance Proximity
# ========================================================================

class TestGraphDistanceProximity:
    """Tests for graph distance proximity."""

    def test_dist_added(self):
        G = _make_overlap_graph()
    ...
    def test_dist_values(self):
        """Adjacent nodes have distance 1, so proximity = 1/1 = 1.0."""
        G = nx.path_graph(4)
        H = graph_distance_proximity(G)
        for u, v, d in H.edges(data=True):
...
# ========================================================================
# Tests: Local Path Index
# ========================================================================

class TestLocalPathIndex:
    """Tests for local path index."""

    def test_lp_added(self):
        G = _make_overlap_graph()
    ...
    def test_lp_values_triangle(self):
        """In a triangle (K_3), each edge has 1 common neighbor (A^2 score)
        and known A^3 counts."""
        G = nx.complete_graph(3)
        H = local_path_index(G, epsilon=0.0)
        # A^2[i][j] = number of common neighbors = 1 for each edge in K_3
        for u, v, d in H.edges(data=True):
            assert d["lp"] == pytest.approx(1.0)

    def test_lp_complete_graph(self):
        G = nx.complete_graph(4)
        H = local_path_index(G, epsilon=0.0)
        for u, v, d in H.edges(data=True):
            # A^2[i][j] = 2 for K_4 (2 common neighbors)
            assert d["lp"] == pytest.approx(2.0)

    def test_epsilon_increases_score(self):
    ...
    def test_path_graph(self):
        ...
        G = nx.path_graph(3)
        H = local_path_index(G, epsilon=0.0)
        # 0-1 and 1-2: no common neighbors, so A^2[i][j] = 0
...
# ========================================================================
# Tests: Cross-index relationships for all proximity indices
# ========================================================================

class TestProximityRelationships:
    """Cross-index properties across all proximity measures."""

    def test_overlap_indices_agree_on_zero_for_bridge(self):
    ...
    def test_ordering_consistent_across_new_indices(self):
    ...
    def test_pa_independent_of_overlap(self):
        """PA only depends on degrees, not common neighbors."""
        G = _make_bridge_graph()
        H = preferential_attachment_score(G)
        # Bridge 2-3: deg(2)=3, deg(3)=3 → PA=9  (each has 2 triangle + 1 bridge)
...
class TestProximityIntegration:
    """Smoke test: run every proximity method on same graphs."""

    def test_all_proximity_undirected(self):
        G = _make_two_cluster_undirected()
        for method, attr in [
    ...
    def test_all_proximity_directed(self):
