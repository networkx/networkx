...
def test_bipartite_color():
    G = nx.path_graph(4)
    c = bipartite.color(G)
    assert c == {0: 1, 1: 0, 2: 1, 3: 0}


def test_not_bipartite_color():
    with pytest.raises(nx.NetworkXError):
        c = bipartite.color(nx.complete_graph(4))


def test_bipartite_directed():
...
def test_bipartite_sets_directed():
    G = nx.path_graph(4)
    D = G.to_directed()
...
def test_bipartite_sets_given_top_nodes():
    G = nx.path_graph(4)
    top_nodes = [0, 2]
...
def test_bipartite_sets_disconnected():
...
def test_is_bipartite_node_set():
    G = nx.path_graph(4)
...
def test_bipartite_weighted_degrees():
    G = nx.path_graph(5)
...
def test_biadjacency_matrix_weight():
...
def test_biadjacency_matrix_order():
    pytest.importorskip("scipy")
    G = nx.path_graph(5)
