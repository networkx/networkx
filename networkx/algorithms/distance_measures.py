...
def extrema_bounding(G, compute="diameter"):
    ...
    minlowernode = max(degrees, key=lambda i: degrees[i])
    N=len(degrees)  # number of nodes
    # alternate between smallest lower and largest upper bound
    high = False
    # status variables
    ecc_lower = dict.fromkeys(G.nodes(), 0)
    ecc_upper = dict.fromkeys(G.nodes(), N)
    candidates = set(G)
    ...
    while candidates:
        ...
        current_ecc = int(max(dist.values()))
...
#        print(ecc_upper)
        ...
        if compute == 'diameter':
            ruled_out = {i for i in candidates \
                if ecc_upper[i] <= maxlower and 2 * ecc_lower[i] >= maxupper}

        elif compute == 'radius':
            ruled_out = {i for i in candidates if ecc_lower[i] >= minupper \
                and ecc_upper[i] + 1 <= 2 * minlower}

        elif compute == 'periphery':
            ruled_out = {i for i in candidates if ecc_upper[i] < maxlower \
...
#        print("NODE 4: %g"%(ecc_upper[4] <= maxlower and 2 * ecc_lower[4] >= maxupper))
