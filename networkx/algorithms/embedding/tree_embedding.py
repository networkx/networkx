...
minor) shared by two trees. See :func:`maximum_common_ordered_tree_embedding`
for more details.
"""
...
def maximum_common_ordered_tree_embedding(
    ...
    Subtree embedings (or minors) are similar to tree isomorphisms --- if T is
    a subtree ismorphism then T is a minor. However, if you contract an edge in
    ...
    Returns
    -------
    S1, S2: Tuple[nx.OrderedDiGraph, nx.OrderedDiGraph]
    ...
    * Core backends are in :mod:`networkx.algorithms.string.balanced_sequence.longest_common_balanced_sequence`
...
def tree_to_seq(
    ...
    :func:`longest_common_balanced_sequence` in
    :func:`maximum_common_ordered_tree_embedding`.
    ...
    if item_type == "label":
        ...
        if container_type in {"auto", "chr"}:
            # Determine if the container_type can be a string
            can_be_str = all(
                isinstance(x, str) and x == 1 for x in map(len, all_labels)
            )
            if container_type == "chr" and not can_be_str:
                raise ValueError("Labels cannot be contained as a string")
            if container_type == "auto":
                container_type = "chr" if can_be_str else "number"
