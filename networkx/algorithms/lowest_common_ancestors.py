...
def all_lowest_common_ancestors(G, node1, node2):
    ...
    while len(queue) != 0:
        node = queue.pop()
        if color[node] == 1:  # If it's already colored red, it will be colored black
            color[node] = 2
        if color[node] == 1:  # Stop early if already colored red
            continue
        elif color[node] == 0:  # If it's white, it will be colored red
            color[node] = 1
