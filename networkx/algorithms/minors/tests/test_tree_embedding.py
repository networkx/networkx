...
def test_null_common_embedding():
    ...
    non_empty = nx.random_ordered_tree(n=1, directed=True)

    with pytest.raises(nx.NetworkXPointlessConcept):
        nx.maximum_common_ordered_subtree_embedding(empty, empty)

    with pytest.raises(nx.NetworkXPointlessConcept):
        nx.maximum_common_ordered_subtree_embedding(empty, non_empty)

    with pytest.raises(nx.NetworkXPointlessConcept):
        nx.maximum_common_ordered_subtree_embedding(non_empty, empty)
...
def test_self_common_embedding():
    ...
    for n in range(1, 10):
        tree = nx.random_ordered_tree(n=n, seed=rng, directed=True)
...
def test_all_implementations_are_same():
    ...
    for _ in range(num_trials):
        ...
        tree1 = nx.random_ordered_tree(n1, seed=rng, directed=True)
        tree2 = nx.random_ordered_tree(n2, seed=rng, directed=True)
        ...
        for impl in impls:
            ...
            embedding1, embedding2, value = nx.maximum_common_ordered_subtree_embedding(
...
def _check_contractions(tree1, tree2, embedding1, embedding2):
    ...
    if 0:
        nx.forest_str(tree1, write=print)
        nx.forest_str(tree2, write=print)
        nx.forest_str(embedding1, write=print)
        nx.forest_str(embedding2, write=print)
        nx.forest_str(embedding1_alt, write=print)
        nx.forest_str(embedding2_alt, write=print)
...
def test_forest_case():
    ...
    F = nx.disjoint_union_all(
        [
            nx.random_ordered_tree(3, seed=0),
            nx.random_ordered_tree(5, seed=1),
            nx.random_ordered_tree(5, seed=1),
            nx.random_ordered_tree(2, seed=2),
            nx.random_ordered_tree(1, seed=3),
        ]
    )

    # Just verify everything runs without error for now. Would be best to find
    # something to check though.
    print(nx.forest_str(F))
