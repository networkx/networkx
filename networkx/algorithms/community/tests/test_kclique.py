...
def test_overlaping_K5():
    ...
    assert_equal(set(c), {frozenset([0, 1, 2, 3, 4]), frozenset([2, 3, 4, 5, 6])})
...
def test_isolated_K5():
    ...
    c= list(nx.k_clique_communities(G, 5))
    assert_equal(set(c), {frozenset([0, 1, 2, 3, 4]), frozenset([5, 6, 7, 8, 9])})

def test_zachary():
    z = nx.karate_club_graph()
    # clique percolation with k=2 is just connected components
    zachary_k2_ground_truth = {frozenset(z.nodes())}
    zachary_k3_ground_truth = {frozenset([0, 1, 2, 3, 7, 8, 12, 13, 14,
                                          15, 17, 18, 19, 20, 21, 22, 23,
                                          26, 27, 28, 29, 30, 31, 32, 33]), frozenset([0, 4, 5, 6, 10, 16]),
                               frozenset([24, 25, 31])}
    zachary_k4_ground_truth = {frozenset([0, 1, 2, 3, 7, 13]), frozenset([8, 32, 30, 33]), frozenset([32, 33, 29, 23])}
    zachary_k5_ground_truth = {frozenset([0, 1, 2, 3, 7, 13])}
