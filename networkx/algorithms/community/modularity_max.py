...
def greedy_modularity_communities(G, weight=None):
    ...
    # Initial modularity
    q_cnm = modularity(G, [[label_for_node[x] for x in c] for c in communities.values()])
    ...
    communities = [frozenset([label_for_node[i] for i in c]) for c in communities.values()]
