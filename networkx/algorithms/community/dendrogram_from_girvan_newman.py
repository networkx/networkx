# -*- coding: utf-8 -*-
"""
This module implements useful functions to plot dendrograms of community detections
performed by the Girvan-Newman algorithm on graphs.
"""
...
__all__ = ['girvan_newman_partitions', 'agglomerative_matrix', 'girvan_newman_best_partition', 'distance_of_partition']
...
def girvan_newman_partitions(G):
    ...
    -------
    list
        List of lists of sets of nodes in `G`. Each set of node
    ...
    # Get list of partitions using `networkx.algorithms.community.centrality.girvan_newman`
    _gn_partitions = list(girvan_newman(G))
    
    # Sort each partition: each partition contains communities (sets of nodes) which will
    # be sorted according to the smallest node they contain. 
    gn_partitions = []
...
def _list2dict(list_partitions, number_nodes):
    ...
    list
        ...
        The dictionary ith contains the information about the partition of the level ith 
...
def _informative_dict(list_of_dict, number_nodes):
    ...
    list_of_dict : list of dictionaries
        List of (`number_nodes` - 1) dictionaries. It is the output of `_list2dict`.
    ...
    list
        ...
        The dictionary ith contains the information about the partition of the level ith 
        and it has this form:
            - key: integrer, label of the node;
    ...
    for dict_i in _list_of_dict:
        dict_pre = dict(dict_cur)  # The old dict with info about the previous partition
        dict_cur = dict(dict_i)    # The old dict with info about the current partition
        
        if c>0:
            newdict_pre = informative_dict[c-1]  # The new dict with info about the 
                                                 # previous partition
            ...
            # Update info of nodes belonging to the new community. Community A and B are 
            # the two communities that are joining together to form the new community.
            ...
            # Create newdict_cur, the new dict with info about the current partition
            ...
            for k,v in newdict_pre.items():
                ...
                if j==info_newcom[2]:
                    break
            if j<info_newcom[2]:
                raise ValueError('ERROR: not found the %d nodes belonging to the new community'
                                 %info_newcom[2])
...
def agglomerative_matrix(G, list_partitions):
    ...
    The Girvan–Newman algorithm detects communities by progressively removing edges 
    from the original graph. The algorithm removes the "most valuable" edge, 
    ...
    The Girvan_Newman algorithm detects (number_nodes - 1) partitions: in particular 
    the first partition will contain 2 communities while in the last one each node
    will be a different community (so, there will be 'number_nodes' different communities).
    The idea is to start from the last partition and retrace the path that unites the 
    communities to the first partition. That is to reconstruct the process of 
    agglomeration of the communities.  
    The "agglomerative matrix" will contain this information.
    ...
    -------
    numpy.ndarray
        The "agglomerative matrix" (AM) is a numpy.ndarray with shape (number_nodes -1)x4.
        The ith row contains information about the new community created at that level 
        by merging 2 existing communities. So the first row contains information about 
        the first new community, which will contain 2 nodes, while the last row contains
        information about the 2 communities which merge to generate the entire graph 
        (with 'number_nodes' nodes).
        The 4 columns contain the following information:
            AM[i,0] = integer, label of the existing community A. The community A will be 
                      merge to the existing community B to form the ith new community.
        ...
        A community with an index less than 'number_nodes' corresponds to one which 
        contains only that single node. A community with an index belonging to 
        [number_nodes, 2*number_nodes - 3] corresponds to one of the new communities 
        formed in the agglomeration process.
    
        This is a matrix of the same type of the one created by the
        `scipy.cluster.hierarchy.linkage` function.
        
        The purpose of the `agglomerative_matrix` function is to create a matrix we can 
    ...
    # Set out the list of partitions in a list of dictionaries containing information on 
    # the agglomeration of communities.
    ...
    for row in range(nn-1):
        ...
        if row < nn-2:
            dict_pre = dict(list_info_dict[row])      # dict of info about previous partition
            dict_cur = dict(list_info_dict[row + 1])  # dict of info about current partition
        
            # Which are the nodes who belong to the new community?
            new_com = nn+row   # Label of the new community created at this level
            n_found = 0
            number_nodes = -1
            nodes_in_newcom = []
            for i in range(nn):                       # Look for all nodes belonging
                if dict_cur[i][0] == new_com:         # to the new community. Their labels
                    nodes_in_newcom.append(i)         # will be stored in nodes_in_newcom
                    if n_found == 0:
            ...
            # Look for info of communities A and B which are merged to form the new community
            if number_nodes < 2:
        ...
        if row==nn-2:
            ...
            if nn_com != nn:
                raise ValueError('ERROR: the last community (which is the entire graph) does not contain "number_nodes" nodes')
...
def girvan_newman_best_partition(G, list_partitions):
    ...
    Returns the best partition among those generated by the Girvan-Newman algorithm.
    The best partition is selected according to modularity, computed using
    `community.community_louvain.modularity`.
    ...
    tupla
        Tupla of 2 elements:
        Fisrt element: dictionary with information about the best partition:
                            - key: integer, label of node;
                            - value: integer, label of community the node belongs to.
        Second element: integer, position of the partition in `list_partitions` which 
                        corresponds to the best partition.
    ...
    Example
    --------
    To get the best partition of `G` among those detected by the Girvan-Newman algorithm::
    ...
    for part in list_partitions:
        ...
        # Compute modularity
        tmp_mod = community_louvain.modularity(tmp_dict, G)
        
        if MAX_mod == 0:                # The first time set `best_partition` equal to `tmp_partition`
            best_partition = tmp_dict
            id_best_part = c
        if tmp_mod>MAX_mod:             # If modularity increases, then update `best_partition`
            MAX_mod = tmp_mod
            best_partition = tmp_dict
            id_best_part = c
...
def distance_of_partition(agglomerative_matrix, n_communities):
    ...
    The partition considered is the one which splits the graph into `n_communities` 
    different communities. 
    
    Parameters
    ----------
    agglomerative_matrix: numpy.ndarray
        Matrix which encodes the hierarchical clustering to render as a dendrogram.
        The hierarchical clustering is detected using the Girvan-Newman algorithm.
        See the `agglomerative_matrix` function for more information on the format of 
        `agglomerative_matrix`.
        
    n_comunities: integer
        The number of communities into which the partition we are interested in is split.
        
    Returns
    -------
    integer
        Distance of the partition we are interested in from the ground level. 
        Which is the distance from the hierarchy level where all nodes are disconnected.
    ...
    Example
    --------
    To get the distance of the partition with 2 communities from the ground level::

        >>> G = nx.path_graph(6)
        >>> partitions = girvan_newman_partitions(G)
        >>> agglomerative_mat = agglomerative_matrix(G, partitions)
        >>> n_communities = 2
        >>> dist_2comm = distance_of_partition(agglomerative_mat, n_communities)
        >>> print (dist_2comm)
        ... 3
        
    To plot the dendrogram highlighting the partition which splits the graph into 2 communities::
    
        >>> from scipy.cluster.hierarchy import dendrogram
        >>> dendro_2comm = dendrogram(agglomerative_mat, color_threshold=dist_2comm)
    ...
    if (n_communities<1) or (n_communities>nn):
        raise ValueError('ERROR: n_communities must be an integer between 1 and number_nodes')
        
    # High of the level of the hierarchy in which the graph is split into `n_communities` different partitions.
