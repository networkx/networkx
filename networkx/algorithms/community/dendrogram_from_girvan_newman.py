...
def girvan_newman_best_partition(G, list_partitions):
    ...
    computed using `community.community_louvain.modularity`.
    ...
    tupla
        ...
        Fisrt element: dictionary with information about the best partition:
                            ...
                            - value: integer, label of community the node
                                     belongs to.
    ...
    for part in list_partitions:
        ...
        if MAX_mod == 0:                # The first time set `best_partition`
            best_partition = tmp_dict   # equal to `tmp_partition`
            id_best_part = c
        if tmp_mod > MAX_mod:           # If modularity increases, then
            MAX_mod = tmp_mod           # update `best_partition`
