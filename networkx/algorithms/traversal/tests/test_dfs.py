...
class TestDFS:
    ...
    def test_preorder_nodes(self):
        ...
        assert_equal(list(nx.dfs_preorder_nodes(self.D, source=2)), [2, 3])
    ...
    def test_postorder_nodes(self):
        ...
        assert_equal(list(nx.dfs_postorder_nodes(self.D, source=0)), [1, 0])

    def test_successor(self):
        assert_equal(nx.dfs_successors(self.G, source=0),
                     {0: [1], 1: [2, 3], 2: [4]})
        assert_equal(nx.dfs_successors(self.G, source=1),
                     {0: [3, 4], 1: [0], 4: [2]})
        assert_equal(nx.dfs_successors(self.D), {0: [1], 2: [3]})
        assert_equal(nx.dfs_successors(self.D, source=1), {1: [0]})
