...
class TestDFS:
    ...
    def dls_test_preorder_nodes(self):
        assert_equal(list(nx.dfs_preorder_nodes(self.G, source=0, search_depth=2)),
            [0, 1, 2, 3])
        assert_equal(list(nx.dfs_preorder_nodes(self.D, source=1, search_depth=2)),
            ([1, 0]))
    ...
    def dls_test_postorder_nodes(self):
        assert_equal(list(nx.dfs_postorder_nodes(self.G, source=1, search_depth=2)),
            [0, 2, 3, 1])
        assert_equal(list(nx.dfs_postorder_nodes(self.D, source=0, search_depth=2)),
            ([1, 0]))
    ...
    def dls_test_successor(self):
        assert_equal(nx.dfs_successors(self.G, source=0, search_depth=2),
                     {0: [1], 1: [2, 3]})
        assert_equal(nx.dfs_successors(self.D, source=0, search_depth=2), {0: [1]})
    ...
    def dls_test_predecessor(self):
        assert_equal(nx.dfs_predecessors(self.G, source=0, search_depth=3),
                     {1: 0, 2: 1, 3: 1, 4: 2})
        assert_equal(nx.dfs_predecessors(self.D, source=0, search_depth=3), {1: 0})
    ...
    def test_dls_tree(self):
        T = nx.dfs_tree(self.G, source=0, search_depth=2)
    ...
    def test_dls_edges(self):
        edges = nx.dfs_edges(self.G, source=0, search_depth=2)
    ...
    def test_dls_labeled_edges(self):
        edges = list(nx.dfs_labeled_edges(self.G, source=0, search_depth=1))
        forward = [(u, v) for (u, v, d) in edges if d['dir'] == 'forward']
        assert_equal(forward, [(0, 0), (0, 1)])
    ...
    def test_dls_labeled_disconnected_edges(self):
        edges = list(nx.dfs_labeled_edges(self.D, source=0, search_depth=2))
        forward = [(u, v) for (u, v, d) in edges if d['dir'] == 'forward']
        assert_equal(forward, [(0, 0), (0, 1)])
