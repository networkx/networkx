...
class TestSingleSourceBMSSPPath:
    def test_simple_path(self):
        ...
        path = single_source_bmssp_path(G, 0, 4)
    ...
    def test_weighted_graph(self):
        ...
        path = single_source_bmssp_path(G, 0, 2)
        assert path == [0, 1, 2]
    ...
    def test_string_nodes(self):
        ...
        path = single_source_bmssp_path(G, "A", "C")
        assert path == ["A", "B", "C"]
...
class TestSingleSourceBMSSPPathLength:
    def test_simple_path(self):
        G = nx.DiGraph(nx.path_graph(5))
        length = single_source_bmssp_path_length(G, 0, 4)
        assert length == 4

    def test_same_node(self):
        G = nx.DiGraph(nx.path_graph(5))
        length = single_source_bmssp_path_length(G, 2, 2)
        assert length == 0
    ...
    def test_weighted_graph(self):
        ...
        length = single_source_bmssp_path_length(G, 0, 2)
        assert length == 7
...
class TestMultiSourceBMSSPPath:
    def test_multiple_sources(self):
        ...
        paths = multi_source_bmssp_path(G, {0, 1})
        assert paths[2] == [1, 2]

    def test_single_source(self):
        G = nx.DiGraph(nx.path_graph(4))
        paths = multi_source_bmssp_path(G, {0})
...
class TestMultiSourceBMSSPPathLength:
    def test_multiple_sources(self):
        ...
        lengths = multi_source_bmssp_path_length(G, {0, 1})
        assert lengths[0] == 0
        assert lengths[1] == 0
        assert lengths[2] == 2
        assert lengths[3] == 5
...
class TestBMSSP:
    ...
    def test_with_target(self):
        ...
        distances, paths = bmssp(G, {0}, target=3)
    ...
    def test_custom_weight_attribute(self):
        ...
        distances, paths = bmssp(G, {"A"}, weight="cost")
...
class TestBMSSPWeightFunction:
    def test_callable_weight(self):
        ...
        def red_only(u, v, d):
            ...
            return None
        distances, paths = bmssp(G, {0}, weight=red_only)
        assert paths[2] == [0, 1, 2]
        assert distances[2] == 2
    ...
    def test_node_weight_function(self):
        ...
        distances, paths = bmssp(G, {0}, weight=func)
