...
class TestAStar:
    ...
    def test_smastar_multiple_optimal_paths(self):
        heuristic_values = {"a": 1.35, "b": 1.18, "c": 0.67, "d": 0}
    ...
    def test_smastar_directed(self):
        assert sma_star_path(self.XG, "s", "v") == ["s", "x", "u", "v"]

    def test_smastar_directed_weight_function(self):
        def w1(u, v, d):
            return d["weight"]
    ...
    def test_smastar_nopath(self):
        with pytest.raises(nx.NodeNotFound):
            sma_star_path(self.XG, "s", "moon")

    def test_smastar_admissible_heuristic_with_cutoff(self):
        heuristic_values = {"s": 36, "y": 4, "x": 0, "u": 0, "v": 0}
    ...
    def test_smastar_inadmissible_heuristic_with_cutoff(self):
        heuristic_values = {"s": 36, "y": 14, "x": 10, "u": 10, "v": 0}
    ...
    def test_smastar_cycle(self):
        C = nx.cycle_graph(7)
        assert sma_star_path(C, 0, 3) == [0, 1, 2, 3]
        assert nx.dijkstra_path(C, 0, 4) == [0, 6, 5, 4]

    def test_smastar_unorderable_nodes(self):
        nodes = [object() for n in range(4)]
        G = nx.Graph()
        G.add_edges_from(pairwise(nodes, cyclic=True))
    ...
    def test_smastar_astar_NetworkXNoPath(self):
        G = nx.gnp_random_graph(10, 0.2, seed=10)
        with pytest.raises(nx.NetworkXNoPath):
            sma_star_path(G, 4, 9)

    def test_smastar_NodeNotFound(self):
        G = nx.gnp_random_graph(10, 0.2, seed=10)
        with pytest.raises(nx.NodeNotFound):
