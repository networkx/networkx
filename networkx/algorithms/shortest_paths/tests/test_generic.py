...
class TestGenericPath:
    ...
    def test_shortest_path(self):
        ...
        assert_equal(nx.shortest_path(self.directed_cycle, 0, 3, weight='weight'),
                     [0, 1, 2, 3])
        # now with ignore nodes and edges
        assert_equal(nx.shortest_path(self.cycle, 0, 3, ignore_nodes=[2]),
                     [0, 6, 5, 4, 3])
        assert_equal(nx.shortest_path(
                     self.cycle, 0, 3, ignore_edges=[(0, 100)]), [0, 1, 2, 3])
        assert_equal(nx.shortest_path(self.cycle, 0, 3, ignore_edges=[(1, 2)]),
                     [0, 6, 5, 4, 3])
        assert_raises(nx.NetworkXNoPath, nx.shortest_path,
                      self.directed_cycle, 0, 3, ignore_nodes=[2])
        assert_raises(nx.NetworkXNoPath, nx.shortest_path,
                      self.directed_cycle, 0, 3, ignore_edges=[(1, 2)])
        assert_equal(nx.shortest_path(self.directed_cycle, 0, 3,
                     ignore_nodes=[5]), [0, 1, 2, 3])
        assert_equal(nx.shortest_path(self.directed_cycle, 0, 3,
                     ignore_edges=[(0, 9)]), [0, 1, 2, 3])

    def test_shortest_path_target(self):
        sp = nx.shortest_path(nx.path_graph(3), target=1)
        assert_equal(sp, {0: [0, 1], 1: [1], 2: [2, 1]})
        # now with ignore nodes and edges
        assert_equal({1: [1]}, nx.shortest_path(
                     nx.path_graph(3), target=1, ignore_nodes=[1]))
        sp = nx.shortest_path(nx.path_graph(3), target=1, ignore_nodes=[2])
        assert_equal(sp, {0: [0, 1], 1: [1]})
        sp = nx.shortest_path(
            nx.path_graph(3), target=1, ignore_edges=[(1, 2)])
        assert_equal(sp, {0: [0, 1], 1: [1]})
        sp = nx.shortest_path(
            nx.path_graph(3), target=1, ignore_edges=[(1, 2), (1, 0)])
        assert_equal(sp, {1: [1]})
    ...
    def test_shortest_path_length(self):
        ...
        assert_equal(nx.shortest_path_length(
                     self.cycle, 0, 3, weight='weight'), 3)
        assert_equal(nx.shortest_path_length(
                     self.grid, 1, 12, weight='weight'), 5)
        assert_equal(nx.shortest_path_length(
                     self.directed_cycle, 0, 4, weight='weight'), 4)
        # now with ignore nodes and edges
        assert_equal(nx.shortest_path_length(
                     self.cycle, 0, 3, ignore_nodes=[2]), 4)
        assert_equal(nx.shortest_path_length(
                     self.cycle, 0, 3, ignore_edges=[(1, 2)]), 4)
        assert_raises(nx.NetworkXNoPath, nx.shortest_path_length,
                      self.directed_cycle, 0, 4, ignore_nodes=[2])
        assert_raises(nx.NetworkXNoPath, nx.shortest_path_length,
                      self.directed_cycle, 0, 4, ignore_edges=[(0, 1)])
        assert_equal(nx.shortest_path_length(
                     self.directed_cycle, 0, 4, ignore_edges=[(1, 0)]), 4)
        assert_equal(nx.shortest_path_length(
                     self.directed_cycle, 0, 4, ignore_edges=[(0, 100)]), 4)
        assert_equal(nx.shortest_path_length(
                     self.directed_cycle, 0, 4, ignore_nodes=[100]), 4)
    ...
    def test_shortest_path_length_target(self):
        ...
        assert_equal(sp[2], 1)
        # now with ignore nodes and edges
        sp = nx.shortest_path_length(nx.path_graph(3), target=1, ignore_nodes=[2])
        assert_true(sp.get(2, True))
        sp = nx.shortest_path_length(nx.path_graph(3), target=1, ignore_edges=[(2, 1)])
        assert_true(sp.get(2, True))
    ...
    def test_single_source_shortest_path(self):
        ...
        assert_equal(p[12], [1, 2, 3, 4, 8, 12])
        # now with ignore nodes and edges
        p=nx.shortest_path(self.cycle, 0, ignore_nodes=[2])
        assert_equal(p[3], [0, 6, 5, 4, 3])
    ...
    def test_single_source_shortest_path_length(self):
        ...
        assert_equal(l[16], 6)
        # now with ignore nodes and edges
        l=nx.shortest_path_length(self.cycle, 0, ignore_nodes=[2])
        assert_equal(l, {0:0, 1:1, 3:4, 4:3, 5:2, 6:1})
        l=nx.shortest_path_length(self.cycle, 0, ignore_edges=[(1, 2)])
        assert_equal(l, {0:0, 1:1, 2:5, 3:4, 4:3, 5:2, 6:1})
    ...
    def test_all_pairs_shortest_path(self):
        ...
        assert_equal(p[1][12], [1, 2, 3, 4, 8, 12])
        # now with ignore nodes and edges
        p=nx.shortest_path(self.cycle, ignore_nodes=[2])
        assert_equal(p[0][3], [0, 6, 5, 4, 3])
        p=nx.shortest_path(self.cycle, ignore_edges=[(2, 3)])
        assert_equal(p[0][3], [0, 6, 5, 4, 3])
    ...
    def test_all_pairs_shortest_path_length(self):
        ...
        assert_equal(l[1][16], 6)
        # now with ignore nodes and edges
        l=nx.shortest_path_length(self.cycle, ignore_nodes=[2])
        assert_equal(l[0], {0:0, 1:1, 3:4, 4:3, 5:2, 6:1})
        l=nx.shortest_path_length(self.cycle, ignore_edges=[(1, 2)])
        assert_equal(l[0], {0:0, 1:1, 2:5, 3:4, 4:3, 5:2, 6:1})
    ...
    def test_has_path(self):
        ...
        # now with ignore nodes and edges
        assert_false(nx.has_path(G, 0, 2, ignore_nodes=[1]))
        assert_false(nx.has_path(G, 0, 2, ignore_edges=[(0, 1)]))
        assert_false(nx.has_path(G, 0, 2, ignore_edges=[(1, 2)]))
