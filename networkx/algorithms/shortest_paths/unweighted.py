...
def single_source_shortest_path_length(
        G, source, cutoff=None, ignore_nodes=[], ignore_edges=[]):
    ...
    if ignore_nodes or ignore_edges:
        with hide_nodes_edges(G, ignore_nodes, ignore_edges):
            return single_source_shortest_path_length(
                G, source=source, cutoff=cutoff)
        ...
        for v in thislevel:
            if v not in seen:
                ...
                for w in G.neighbors_iter(v):
                    nextlevel[w] = 1  # add neighbors of v
...
def all_pairs_shortest_path_length(
        G, cutoff=None, ignore_nodes=[], ignore_edges=[]):
    ...
    for n in G:
        if n not in ignore_nodes:
            paths[n] = single_source_shortest_path_length(
                G, n, cutoff=cutoff, ignore_nodes=ignore_nodes,
                ignore_edges=ignore_edges)
    return paths


def bidirectional_shortest_path(
        G, source, target, ignore_nodes=[], ignore_edges=[]):
...
def _bidirectional_pred_succ(
    ...
    if ignore_nodes or ignore_edges:
        with hide_nodes_edges(G, ignore_nodes, ignore_edges):
            return _bidirectional_pred_succ(G, source=source, target=target)
...
def single_source_shortest_path(G, source, cutoff=None,
    ...
    if ignore_nodes or ignore_edges:
        with hide_nodes_edges(G, ignore_nodes, ignore_edges):
            return single_source_shortest_path(G, source=source, cutoff=cutoff)
...
def all_pairs_shortest_path(G, cutoff=None,
    ...
    for n in G:
        if n not in ignore_nodes:
            paths[n] = single_source_shortest_path(G, n, cutoff=cutoff, ignore_nodes=ignore_nodes, ignore_edges=ignore_edges)
    return paths
...
def predecessor(G, source, target=None, cutoff=None, return_seen=None,
    ...
    if ignore_nodes or ignore_edges:
        with hide_nodes_edges(G, ignore_nodes, ignore_edges):
            return predecessor(G, source=source, target=target, cutoff=cutoff, return_seen=return_seen)
