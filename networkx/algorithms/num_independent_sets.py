...
def num_independent_sets(G):
    ...
    num_ind_sets_at_node = {}

    for node in postorder_nodes:
        # For undirected graphs, only get nodes that are already visited (children)
        children = {
            node
            for node in _get_tree_neighbors(G, {node})
            if node in num_ind_sets_at_node
        }  # get children of the node
        grandchildren = {
            node
            for node in _get_tree_neighbors(G, children)
            if node in num_ind_sets_at_node
        }  # get grandchildren of the node
        ...
        if len(children) == 0 and len(grandchildren) == 0:
            ...
            num_ind_sets_at_node[node] = (
                math.prod([num_ind_sets_at_node[c] for c in children]) + 1
            )
        else:
            ...
            num_ind_sets_at_node[node] = math.prod(
                [num_ind_sets_at_node[c] for c in children]
