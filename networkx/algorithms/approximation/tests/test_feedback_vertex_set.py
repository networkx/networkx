...
from networkx.algorithms.approximation import minimum_feedback_vertex_set
...
class TestFeedbackVertexSet():
    def test_cycle(self):
        G = nx.cycle_graph(10)
        F = minimum_feedback_vertex_set(G)
    ...
    def test_weighted_cycle(self):
        ...
        F = minimum_feedback_vertex_set(G, w)
        assert_equal(F, [9]) 
    ...
    def test_weighted_complete(self):
        ...
        w = {v:(2.0 - 0.1*v) for v in G.nodes()}
        F = minimum_feedback_vertex_set(G, w)
        assert_equal(sorted(F), list(range(2,10)))  
    ...
    def test_weights_exception(self):
        ...
        assert_raises(NetworkXError, minimum_feedback_vertex_set, G, w)

    def test_digraph(self):
        G = nx.DiGraph()
        assert_raises(NetworkXNotImplemented, minimum_feedback_vertex_set, G)
