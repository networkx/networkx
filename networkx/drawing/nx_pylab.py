...
def draw_weighted(G, pos=None, ax=None, hold=None, alpha=1.,
                  edgelist=None, edge_cmap='bwr', edge_vmin=None, edge_vmax=None,
                  **kwds):
    """Convenience function to draw the weighted graph G with Matplotlib.

    Draw the weighted graph as a simple representation with no node
    labels or edge labels and using the full Matplotlib figure area
    and no axis labels by default.  See draw_networkx() for more
    full-featured drawing that allows title, axis labels etc.

    Weights are represented by the color of the edges.
    The argument 'edge_color' is hence overwritten by this function.
    ...
    edge_cmap: any divergent matplotlib.colormap specification (default: 'bwr')
        divergent colormap used to represent the weights;
            - negative weights are mapped to first half of colormap
            - zero weights are mapped to midpoint
            - positive weights are mapped to second half of colormap
        good choices include: 'bwr', 'RdGy',

    edge_vmin, edge_vmax : floats
       Minimum and maximum for edge colormap scaling (default=None)

    alpha : float
       The node and edge transparency (default=1.0)
    ...
    >>> G=nx.dodecahedral_graph()
    >>> nx.draw_weighted(G)
    >>> nx.draw_weighted(G,pos=nx.spring_layout(G)) # use spring layout
    ...
    # --------------------------------------------------------------------------------
    # everything outside lines as in draw()
    # this functionality could be integrated into draw by adding a 'weighted' flag.
    ...
    # get edge weights
    weights = numpy.array([G[u][v]['weight'] for (u,v) in edgelist])

    # reorder edges such that edges with large absolute weights are plotted last
    # and hence most prominent in the graph
    order = numpy.argsort(numpy.abs(weights))
    edgelist = [edgelist[ii] for ii in order]
    weights = weights[order]

    # apply edge_vmin, edge_vmax
    if edge_vmin == None:
        edge_vmin = numpy.min(weights)
    if edge_vmax == None:
        edge_vmax = numpy.max(weights)
    ...
    weights /= numpy.max(numpy.abs(weights)) # [-1, 1]
    weights += 1. # [0, 2]
    weights /= 2. # [0, 1]

    # convert edges to colors
    edge_color = _handle_colors(weights,
                                alpha=alpha,
                                elem_list=edgelist,
                                cmap=edge_cmap,
                                vmin=0.,
    ...
    if pos is None and numpy.any(weights < 0):
        weighted_edges = [(u, v, w) for (u,v), w in zip(edgelist, numpy.abs(weights))]
        if G.is_directed():
            Gpos = nx.DiGraph()
        else:
            Gpos = nx.Graph()
    ...
    # update kwds to push changes
    #  - edgelist is passed on
    #  - edge_color is overwritten
    #  - edge_vmin, edge_vmax, edge_cmap, are 'caught' and not passed on
    # handle_colors should identify the fully specified edge_color and not touch that variable
    kwds.update(dict(edgelist=edgelist,
