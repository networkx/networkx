...
def kamada_kawai(G, pos=None, K=1.0, L=0.1, iterations=None,
                 tolerance=1, D=0.0, weight='weight'):
        ...
        for v in range(n):
            if u != v:
                if math.isinf(dist[u, v]):
                    dist[u, v] = diameter
                k[u, v] = K / (dist[u, v] ** 2.0)
                l[u, v] = L * dist[u, v]

    def compute_derivatives(pxx, pxy, pyx, pyy, kappa, la):
        diff_x = pxx - pyx
        diff_y = pxy - pyy
        eu_dist = math.sqrt(diff_x ** 2.0 + diff_y ** 2.0)
        eu_dist_cubed = eu_dist ** 3.0
        edx = kappa * (diff_x - (la * diff_x / eu_dist))
        edy = kappa * (diff_y - (la * diff_y / eu_dist))
        edxx = kappa * (1 - (la * (diff_y ** 2.0)) / eu_dist_cubed)
        edyy = kappa * (1 - (la * (diff_x ** 2.0)) / eu_dist_cubed)
    ...
    derivatives = np.zeros((pos_arr.shape[0], number_of_derivatives),
                           dtype=dist.dtype)
    partial_derivatives = np.vectorize(compute_derivatives)
    for u in range(n):
        elements = np.arange(n)
        elements = elements[elements != u]
        derivatives_m = np.sum(np.array(list(
            partial_derivatives(pos_arr[u][0], pos_arr[u][1], pos_arr[elements, 0],
                                pos_arr[elements, 1], k[u, elements],
                                l[u, elements])),
            dist.dtype).reshape(number_of_derivatives, n - 1), axis=1)
        derivatives[u] = derivatives_m
        delta[u] = math.sqrt(derivatives_m[0] ** 2.0 + derivatives_m[1] ** 2.0)
    ...
    while delta[m] > D and (outer_iter < iterations):
        ...
        while delta[m] > D and (inner_iter < tolerance):
            ...
            dy = (derivatives[m, 2] * derivatives[m, 1] - derivatives[m, 4]
                  * derivatives[m, 0]) / (derivatives[m, 4] ** 2.0 -
            ...
            derivatives_m = np.sum(np.array(list(
                partial_derivatives(pos_arr[m, 0], pos_arr[m, 1], pos_arr[elements, 0],
                                    pos_arr[elements, 1], k[m, elements],
                                    l[m, elements])),
                                    dist.dtype).reshape(
                                    number_of_derivatives, n - 1), axis=1)
            derivatives[m] = derivatives_m
            delta[m] = math.sqrt(derivatives_m[0] ** 2.0 + derivatives_m[1] ** 2.0)

        # Compute new partial derivatives for every node
        for u in range(n):
            if u != m:
                new_dx, new_dy, new_dxx, new_dyy, new_dxy = \
                    compute_derivatives(pos_arr[u, 0], pos_arr[u, 1],
                                        pos_arr[m, 0], pos_arr[m, 1], k[u, m], l[u, m])
                old_dx, old_dy, old_dxx, old_dyy, old_dxy = \
                    compute_derivatives(pos_arr[u, 0], pos_arr[u, 1],
                                        old_px, old_py, k[u, m], l[u, m])
                derivs_u = derivatives[u]
                derivatives[u] = [derivs_u[0] + (new_dx - old_dx),
                                  derivs_u[1] + (new_dy - old_dy),
                                  derivs_u[2] + (new_dxx - old_dxx),
                                  derivs_u[3] + (new_dyy - old_dyy),
                                  derivs_u[4] + (new_dxy - old_dxy)]
                delta[u] = math.sqrt(derivs_u[0] ** 2.0 + derivs_u[1] ** 2.0)
        m = np.argmax(delta)
