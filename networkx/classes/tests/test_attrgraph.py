...
class TestGraph:
    ...
    def setUp(self):
        self.Graph=networkx.AttrGraph
        # build K3
        self.k3adj={0: {1: {'data':1}, 2: {'data':1}}, 1: {0: {'data':1}, 2: {'data':1}}, 2: {0: {'data':1}, 1: {'data':1}}}
        self.k3edges=[(0, 1), (0, 2), (1, 2)]
        self.k3nodes=[0, 1, 2]
        self.K3=self.Graph(weighted=True)
    ...
    def test_data_input(self):
        G=self.Graph(data={1:[2],2:[1]}, name="test",weighted=True)
        assert_equal(G.name,"test")
        assert_equal(sorted(G.adj.items()),[(1, {2: {'data':1}}), (2, {1: {'data':1}})])
        G=self.Graph({1:[2],2:[1]}, name="test",weighted=True)
        assert_equal(G.name,"test")
        assert_equal(sorted(G.adj.items()),[(1, {2: {'data':1}}), (2, {1: {'data':1}})])
    ...
    def test_getitem(self):
        G=self.K3
        assert_equal(G[0],{1: {'data':1}, 2: {'data':1}})
    ...
    def test_remove_node(self):
        G=self.K3
        G.remove_node(0)
        assert_equal(G.adj,{1:{2:{'data':1}},2:{1:{'data':1}}})
    ...
    def test_add_edge(self):
        G=self.Graph(weighted=True)
        G.add_edge(0,1)
        assert_equal(G.adj,{0: {1: {'data':1}}, 1: {0: {'data':1}}})
        G=self.Graph(weighted=True)
        G.add_edge(*(0,1))
        assert_equal(G.adj,{0: {1: {'data':1}}, 1: {0: {'data':1}}})
        G=self.Graph()
    ...
    def test_add_edges_from(self):
        ...
        assert_equal(G.adj,{0: {1:{}, 2:{'data':3}}, 1: {0:{}}, 2:{0:{'data':3}}})
        G.add_edges_from([(0,1),(0,2,3)],data=2)
        assert_equal(G.adj,{0: {1:{'data':2}, 2:{'data':3}}, 1: {0:{'data':2}}, 2:{0:{'data':3}}})
        G=self.Graph(weighted=True)
        G.add_edges_from([(0,1),(0,2,3)])
        assert_equal(G.adj,{0: {1:{'data':1}, 2:{'data':3}}, 1: {0:{'data':1}}, 2:{0:{'data':3}}})
        G.add_edges_from([(0,1),(0,2,3)],data=2)
        assert_equal(G.adj,{0: {1:{'data':2}, 2:{'data':3}}, 1: {0:{'data':2}}, 2:{0:{'data':3}}})

        assert_raises((TypeError,networkx.NetworkXError), G.add_edges_from,[(0,)])  # too few in tuple
        assert_raises((TypeError,networkx.NetworkXError), G.add_edges_from,[(0,1,2,3)])  # too many in tuple
    ...
    def test_remove_edge(self):
        G=self.K3
        G.remove_edge(0,1)
        assert_equal(G.adj,{0:{2:{'data':1}},1:{2:{'data':1}},2:{0:{'data':1},1:{'data':1}}})
...
#        assert_equal(G.has_edge(0,1,2),True)
    ...
    def test_get_edge_data(self):
        G=self.K3
        assert_equal(G.get_edge_data(0,1),{'data':1})
        assert_equal(G[0][1],{'data':1})
...
#        assert_raises((KeyError,networkx.NetworkXError), G.get_edge_data,-1,0)
    ...
    def test_adjacency_iter(self):
        ...
        assert_equal(dict(G.adjacency_iter()),
                {0: {1: {'data':1}, 2: {'data':1}}, 1: {0: {'data':1}, 2: {'data':1}}, 2: {0: {'data':1}, 1: {'data':1}}})
    ...
    def test_selfloops(self):
        ...
        assert_equal(G.selfloop_edges(data=True),[(0,0,{'data':1})])
    ...
    def test_graph_attr(self):
        ...
        del G.graph['foo']
        assert_equal(G.graph['foo'],{})  # Is this what we want defaultdict to return?

    def test_node_attr(self):
        G=self.Graph()
        G.add_node(1,foo='bar')
        assert_equal(G.node[1],{'foo':'bar'})
        assert_equal(G.nodes(), [1])
        assert_equal(G.nodes(attr=True),{1:{'foo':'bar'}})
        G.node[1]['foo']='baz'
        assert_equal(G.nodes(attr=True),{1:{'foo':'baz'}})

    def test_edge_attr(self):
        G=self.Graph(weighted=True)
        G.add_edge(1,2,foo='bar')
        assert_equal(G.edges(), [(1,2)])
        assert_equal(G.edges(attr=True), [(1,2,{'data':1,'foo':'bar'})])
        G=self.Graph()
        G.add_edge(1,2,foo='bar')
        assert_equal(G.edges(attr=True), [(1,2,{'foo':'bar'})])
        # edge 3-4 and 4-5 get same attribute
        G.add_edges_from(((3,4),(4,5)),foo='foo')
        assert_equal(G.edges(attr=True), [(1,2,{'foo':'bar'}),(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
        G.remove_edge(1,2)
        assert_equal(G.edges(attr=True), [(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
        G.add_edge(1,2,data=7,foo='bar',bar='foo')
        assert_equal(G.edges(attr=True), [(1,2,{'data':7,'foo':'bar','bar':'foo'}),(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
        G[1][2]['data']=10 # OK to set data like this
        assert_equal(G.edges(attr=True), [(1,2,{'data':10,'foo':'bar','bar':'foo'}),(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
        G.edge[1][2]['data']=20 # another spelling, "edge"
        assert_equal(G.edges(attr=True), [(1,2,{'data':20,'foo':'bar','bar':'foo'}),(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
        G.edge[1][2]['listdata']=[20,200] 
        assert_equal(G.edges(attr=True), [(1,2,{'listdata':[20,200],'data':20,'foo':'bar','bar':'foo'}),(3,4,{'foo':'foo'}),(4,5,{'foo':'foo'})])
