...
def _random_walker(G, a, b, start_node):
    choice = np.random.choice(2, p=[a, b])
    if choice == 0:
...
def random_walkers_graph(m, p, a, b, steps):
    """Return random graph generated by random walkers.
    ...
    p : float
        Probability for edge creation for the seed graph
    a : float
        Probability for discovering a new node for random walkers
    b : float
        Probability for walking to a second neighbor to creat an edge
    ...
    Notes
    -----
    The initialization is a gnp random graph with n nodes.

    References
    ----------

    """
    G = fast_gnp_random_graph(n, p, seed=None, directed=False)
    time = 0
    for (n1, n2) in G.edges_iter(data=1):
        G[n1][n2]['start'] = time
