...
def is_threshold_sequence(degree_sequence):
    ...
    """
    ds = sorted(degree_sequence[:])  # get a copy so we don't destroy original
    while ds:
...
def find_creation_sequence(G):
    ...
    while H.order() > 0:
        ...
        dsdict = H.degree()
        ds = sorted([[d, v] for v, d in dsdict.items()])
...
def triangles(creation_sequence):
    ...
    # number of triangles in clique of nd d's
    ntri = dr * (dr - 1) * (dr - 2) / 6
...
def triangle_sequence(creation_sequence):
    ...
    # number of triangles through a node of clique dr
    dcur = (dr - 1) * (dr - 2) // 2
...
def eigenvalues(creation_sequence):
    ...
    """
    degseq = sorted(degree_sequence(creation_sequence))
...
def swap_d(cs, p_split=1.0, p_combine=1.0, seed=None):
    ...
    if random.random() < p_split:
        ...
        if split_to != flip_side and cs[
                split_to] == 'i' and cs[flip_side] == 'i':
            cs[choice] = 'i'
            cs[split_to] = 'd'
            cs[flip_side] = 'd'
            dlist.remove(choice)
...
#            print >>sys.stderr,"split at %s to %s and %s"%(choice,split_to,flip_side)
    ...
    if random.random() < p_combine and dlist:
        ...
        if target >= len(
                cs) or cs[target] == 'd' or first_choice == second_choice:
            return cs
        # OK to combine
        cs[first_choice] = 'i'
        cs[second_choice] = 'i'
