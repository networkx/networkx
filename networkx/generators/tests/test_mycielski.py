...
class TestMycielski(object):
    ...
    def test_construction(self):
        G = nx.path_graph(2)
        M = nx.mycielskian(G)
        assert_true(nx.is_isomorphic(M, nx.cycle_graph(5)))
    ...
    def test_mycielski_graph_generator(self):
        ...
        assert_true(nx.is_isomorphic(G, nx.empty_graph(1)))
        G = nx.mycielski_graph(2)
        assert_true(nx.is_isomorphic(G, nx.path_graph(2)))
        G = nx.mycielski_graph(3)
        assert_true(nx.is_isomorphic(G, nx.cycle_graph(5)))
        G = nx.mycielski_graph(4)
        assert_true(nx.is_isomorphic(G, nx.mycielskian(nx.cycle_graph(5))))
