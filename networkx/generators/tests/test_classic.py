...
class TestGeneratorClassic():
    ...
    def test_balanced_tree_star(self):
        ...
        assert_true(is_isomorphic(t, nx.star_graph(2)))
        t = nx.balanced_tree(r=5, h=1)
        assert_true(is_isomorphic(t, nx.star_graph(5)))
        t = nx.balanced_tree(r=10, h=1)
        assert_true(is_isomorphic(t, nx.star_graph(10)))
    ...
    def test_full_rary_tree_path(self):
        ...
        assert_true(is_isomorphic(t, nx.path_graph(10)))

    def test_full_rary_tree_empty(self):
        t = nx.full_rary_tree(0, 10)
        assert_true(is_isomorphic(t, nx.empty_graph(10)))
        t = nx.full_rary_tree(3, 0)
        assert_true(is_isomorphic(t, nx.empty_graph(0)))
    ...
    def test_barbell_graph(self):
        ...
        assert_true(nx.number_of_nodes(b) == 2 * m1 + m2)
        assert_true(nx.number_of_edges(b) == m1 * (m1 - 1) + m2 + 1)
        ...
        m1 = 1
        m2 = 20
        assert_raises(nx.NetworkXError, nx.barbell_graph, m1, m2)
        ...
        m2 = 5
        b = nx.barbell_graph(m1, m2)
        assert_true(is_isomorphic(b, nx.path_graph(m2 + 4)))
    ...
    def test_binomial_tree(self):
        for n in range(0,4):
            ...
            assert_true(nx.number_of_nodes(b) == 2**n)
            assert_true(nx.number_of_edges(b) == (2**n - 1))
    ...
    def test_complete_graph(self):
        ...
        for m in [0, 1, 3, 5]:
            ...
            assert_true(nx.number_of_nodes(g) == m)
            assert_true(nx.number_of_edges(g) == m * (m - 1) // 2)
    ...
    def test_complete_digraph(self):
        ...
        for m in [0, 1, 3, 5]:
            ...
            assert_true(nx.number_of_nodes(g) == m)
            assert_true(nx.number_of_edges(g) == m * (m - 1))
    ...
    def test_circular_ladder_graph(self):
        ...
        assert_raises(nx.NetworkXError, nx.circular_ladder_graph,
    ...
    def test_dorogovtsev_goltsev_mendes_graph(self):
        ...
        assert_equal(nx.average_clustering(G), 1.0)
        assert_equal(sorted(nx.triangles(G).values()), [1, 1, 1])
        G = nx.dorogovtsev_goltsev_mendes_graph(10)
        assert_equal(nx.number_of_nodes(G), 29526)
        assert_equal(nx.number_of_edges(G), 59049)
        assert_equal(G.degree(0), 1024)
        assert_equal(G.degree(1), 1024)
        assert_equal(G.degree(2), 1024)

        assert_raises(nx.NetworkXError,
                      nx.dorogovtsev_goltsev_mendes_graph, 7,
                      create_using=nx.DiGraph)
        assert_raises(nx.NetworkXError,
                      nx.dorogovtsev_goltsev_mendes_graph, 7,
    ...
    def test_create_using(self):
        G = nx.empty_graph()
        assert_true(isinstance(G, nx.Graph))
        assert_raises(TypeError, nx.empty_graph, create_using=0.0)
        assert_raises(TypeError, nx.empty_graph, create_using="Graph")

        G = nx.empty_graph(create_using=nx.MultiGraph)
        assert_true(isinstance(G, nx.MultiGraph))
        G = nx.empty_graph(create_using=nx.DiGraph)
        assert_true(isinstance(G, nx.DiGraph))
    ...
    def test_empty_graph(self):
        ...
        G = nx.empty_graph(42)
        assert_equal(nx.number_of_nodes(G), 42)
    ...
    def test_ladder_graph(self):
        for i, G in [(0, nx.empty_graph(0)), (1, nx.path_graph(2)),
            ...
            assert_true(is_isomorphic(nx.ladder_graph(i), G))

        assert_raises(nx.NetworkXError,
                      nx.ladder_graph, 2, create_using=nx.DiGraph)
    ...
    def test_lollipop_graph(self):
        ...
        for m1, m2 in [(3, 5), (4, 10), (3, 20)]:
            ...
            assert_equal(nx.number_of_nodes(b), m1 + m2)
            assert_equal(nx.number_of_edges(b), m1 * (m1 - 1) / 2 + m2)

        # Raise NetworkXError if m<2
        assert_raises(nx.NetworkXError,
                      nx.lollipop_graph, 1, 20)

        # Raise NetworkXError if n<0
        assert_raises(nx.NetworkXError,
                      nx.lollipop_graph, 5, -2)

        # lollipop_graph(2,m) = path_graph(m+2)
        for m1, m2 in [(2, 5), (2, 10), (2, 20)]:
            b = nx.lollipop_graph(m1, m2)
            assert_true(is_isomorphic(b, nx.path_graph(m2 + 2)))
    ...
    def test_null_graph(self):
        assert_equal(nx.number_of_nodes(nx.null_graph()), 0)

    def test_path_graph(self):
        p = nx.path_graph(0)
        assert_true(is_isomorphic(p, nx.null_graph()))

        p = nx.path_graph(1)
        assert_true(is_isomorphic(p, nx.empty_graph(1)))

        p = nx.path_graph(10)
        assert_true(nx.is_connected(p))
    ...
    def test_star_graph(self):
        ...
        assert_true(is_isomorphic(star_graph(0), nx.empty_graph(1)))
        assert_true(is_isomorphic(star_graph(1), nx.path_graph(2)))
        assert_true(is_isomorphic(star_graph(2), nx.path_graph(3)))
    ...
    def test_trivial_graph(self):
        assert_equal(nx.number_of_nodes(nx.trivial_graph()), 1)

    def test_turan_graph(self):
        assert_equal(nx.number_of_edges(nx.turan_graph(13, 4)), 63)
        assert_true(is_isomorphic(nx.turan_graph(13, 4),
