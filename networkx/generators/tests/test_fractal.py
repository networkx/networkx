...
def test_laakso_graph(create_using):
    ...
    returned_pos = set([tuple(list(arr)) for arr in nx.get_node_attributes(G, "pos").values()])
    expected_pos = set([(0, 0), (0, 1)])
    ...
    assert nx.is_isomorphic(G, H)
    returned_pos = set([tuple(list(arr)) for arr in nx.get_node_attributes(G, "pos").values()])
    expected_pos = set([(-0.25, 0.5), (0.0, 0.0), (0.0, 0.25), (0.0, 0.75), (0.0, 1.0), (0.25, 0.5)])
    assert returned_pos == expected_pos
...
def test_sierpinski_gasket_graph(create_using):
    ...
    # normalize
    expected_pos = set([(x/2, (y + np.sqrt(3)/2)/2) for (x, y) in expected_pos])
    assert returned_pos == expected_pos
