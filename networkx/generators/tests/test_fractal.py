...
def test_laakso_graph(create_using):
    ...
    G = nx.laakso_graph(1, create_using=create_using)
    ...
    G = nx.laakso_graph(2, create_using=create_using)
    H = nx.from_edgelist(
        [
            (0, 1),
            (1, 2),
...
def test_sierpinski_gasket_graph(create_using):
    ...
    G = nx.sierpinski_gasket_graph(1, create_using=create_using)
    ...
    G = nx.sierpinski_gasket_graph(2, create_using=create_using)
    H = nx.from_edgelist(
        [
            (0, 1),
            (1, 2),
