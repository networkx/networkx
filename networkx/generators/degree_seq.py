...
def directed_configuration_model(in_degree_sequence,
    ...
    G.name = "directed configuration_model %d nodes %d edges" % (
        G.order(), G.size())
    return G
...
def directed_havel_hakimi_graph(in_deg_sequence,
    ...
    sumin, sumout, nin, nout = 0, 0, len(
        in_deg_sequence), len(out_deg_sequence)
    maxn = max(nin, nout)
    ...
    for n in range(maxn):
        ...
        sumin, sumout, maxin = sumin + \
            in_deg, sumout + out_deg, max(maxin, in_deg)
        if in_deg > 0:
    ...
    G.name = "directed_havel_hakimi_graph %d nodes %d edges" % (
        G.order(), G.size())
    return G
